        ...
        <div className="space-y-4">
          ...
          {(q.type === "mcq" || q.type === "checkbox") && (
            <div>
              ...
              {/* Validation message for insufficient options */}
              {q.options.length < 2 && (
                <div className="flex items-center gap-2 text-sm text-red-600 dark:text-red-400 mt-3">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                  </svg>
                  At least 2 options are required for {q.type === "mcq" ? "multiple choice" : "checkbox"} questions
                </div>
              )}
            </div>
          )}
...
export default function AdminCreatePage() {
  ...
  function handleAddQuestion() {
    ...
    setQuestions([...questions, { id: newId, type: "yesno", label: "", options: [] }]);
  }
  ...
  const isFormReady = () => {
    return (
      ...
      questions.every(q => {
        // For checkbox and mcq questions, require at least 2 options
        if (q.type === "checkbox" || q.type === "mcq") {
